...
- 第二轮前端快照规格通过后，质量复审发现预览身份失效和首页 GET 身份 fence 两项阻塞；前端 `ab18718` 完成 RED→GREEN，定向 22/22、相关 99/99、全量 1140/1140 与 production build/chunk 通过。旧前端快照及配对后端快照失效；后端原快照的 SPEC/SECURITY 曾通过，TEST 在配对变更后中止，等待新快照完整重审。
