...
[[ -f "$BACKEND_DIR/.env.example" ]] || { echo "missing backend environment example after reset: $BACKEND_DIR/.env.example" >&2; exit 1; }
[[ -x "$BACKEND_DIR/deploy/merge-env-example.sh" ]] || { echo "missing environment merge script after reset" >&2; exit 1; }
[[ -x "$BACKEND_DIR/deploy/production-deploy.sh" ]] || { echo "missing backend deployment script after reset" >&2; exit 1; }
[[ -f "$FRONTEND_DIR/package.json" ]] || { echo "missing frontend package.json after reset: $FRONTEND_DIR/package.json" >&2; exit 1; }
[[ -f "$FRONTEND_DIR/package-lock.json" ]] || { echo "missing frontend package-lock.json after reset: $FRONTEND_DIR/package-lock.json" >&2; exit 1; }
[[ -f "$FRONTEND_DIR/.env.example" ]] || { echo "missing frontend environment example after reset: $FRONTEND_DIR/.env.example" >&2; exit 1; }
