...
type scriptedUserDeleteBackend struct {
	...
	executeCalls int
	queryCalls   int
	issue        service.VerificationIssue
	begin        service.OperationBegin
	queryActor   service.ActionActor
	queryKeys    []string
}

func (s *scriptedUserDeleteBackend) Issue(_ context.Context, in service.VerificationIssue) (*service.IssuedVerification, error) {
	s.issueCalls++
	in.CurrentPassword = append([]byte(nil), in.CurrentPassword...)
	s.issue = in
...
func (s *scriptedUserDeleteBackend) Query(_ context.Context, _ actionsecurity.Action, actor service.ActionActor, keys []string) (*service.OperationView, error) {
	s.queryCalls++
	s.queryActor = actor
...
func TestAdminUserActionIssueBuildsTrustedInputAndExactSuccess(t *testing.T) {
	...
	intent, ok := backend.issue.Intent.(actionsecurity.DeleteUserIntent)
	if backend.issueCalls != 1 || !ok || intent.TargetGUID != 123 || intent.ExpectedAuthVersion != 7 || intent.Reason != "duplicate account" ||
		backend.issue.Actor != (service.ActionActor{UserID: 17, UserGUID: 1701, AuthVersion: 9, SessionSID: "11111111-2222-4333-8444-555555555555", SessionVersion: 4}) ||
		backend.issue.TrustedIP != "203.0.113.8" || string(backend.issue.CurrentPassword) != "actor-secret" {
		t.Fatalf("unexpected issue input: %#v intent=%#v", backend.issue, intent)
	}
}
...
func TestAdminUserActionIssueRejectsAnySensitiveHeadersBeforeService(t *testing.T) {
	for _, headers := range []http.Header{
		...
		if rec.Code != 400 || backend.issueCalls != 0 || !strings.Contains(rec.Body.String(), `"code":"invalid_admin_action_request"`) {
			t.Fatalf("headers=%v status/body/calls=%d %s %d", headers, rec.Code, rec.Body.String(), backend.issueCalls)
		}
	}
}
...
func TestAdminUserActionExecuteUsesRawHeadersOnceAndDoesNotReplay(t *testing.T) {
	...
	intent, ok := backend.begin.Intent.(actionsecurity.DeleteUserIntent)
	if backend.beginCalls != 1 || backend.executeCalls != 1 || !ok || intent != (actionsecurity.DeleteUserIntent{TargetGUID: 123, ExpectedAuthVersion: 7, Reason: "duplicate account"}) ||
		backend.begin.Actor != (service.ActionActor{UserID: 17, UserGUID: 1701, AuthVersion: 9, SessionSID: "11111111-2222-4333-8444-555555555555", SessionVersion: 4}) ||
		len(backend.begin.IdempotencyKeyValues) != 1 || backend.begin.IdempotencyKeyValues[0] != testActionKey || len(backend.begin.TicketValues) != 1 || backend.begin.TicketValues[0] != testActionTicket {
		t.Fatalf("unexpected calls/input: begin=%d execute=%d input=%#v", backend.beginCalls, backend.executeCalls, backend.begin)
	}
}
...
func TestAdminUserActionQueryIsExactAndSetsRetryAfterOnlyForProcessing(t *testing.T) {
	...
	if rec.Code != 200 || rec.Header().Get("Retry-After") != "7" || rec.Body.String() != `{"operation_ref":"`+testOperationRef+`","scope":"users.delete","status":"processing","finished_at":null,"failure_code":null}` || backend.queryCalls != 1 {
		t.Fatalf("status/body/headers/calls=%d %s %v %d", rec.Code, rec.Body.String(), rec.Header(), backend.queryCalls)
	}
	if backend.queryActor != (service.ActionActor{UserID: 17, UserGUID: 1701, AuthVersion: 9, SessionSID: "11111111-2222-4333-8444-555555555555", SessionVersion: 4}) || len(backend.queryKeys) != 1 || backend.queryKeys[0] != testActionKey {
		t.Fatalf("query actor/keys=%#v %#v", backend.queryActor, backend.queryKeys)
	}
	...
	if rec.Code != 200 || retryPresent || rec.Body.String() != `{"operation_ref":"`+testOperationRef+`","scope":"users.delete","status":"succeeded","finished_at":1790000000000,"failure_code":null}` {
		t.Fatalf("terminal status/body/headers=%d %s %v", rec.Code, rec.Body.String(), rec.Header())
	}
}
...
func TestAdminUserActionExecuteRejectsEveryMalformedHeaderShapeBeforeBegin(t *testing.T) {
	...
	for _, headers := range tests {
		...
		if rec.Code != 400 || backend.beginCalls != 0 {
			t.Fatalf("headers=%v status/body/begin=%d %s %d", headers, rec.Code, rec.Body.String(), backend.beginCalls)
		}
	}
}
...
func TestAdminUserActionErrorMappingIsExhaustiveAndRedacted(t *testing.T) {
	...
	for _, test := range tests {
		...
		if r.Code != test.status || r.Body.String() != want || strings.Contains(r.Body.String(), "private") {
			t.Fatalf("err=%v status/body=%d %s want=%d %s", test.err, r.Code, r.Body.String(), test.status, want)
		}
	}
}
...
func TestAdminUserActionRateLimitAndCommitUnknownExposeOnlySafeMetadata(t *testing.T) {
	...
	if issue.Code != 429 || issue.Header().Get("Retry-After") != "13" || !strings.Contains(issue.Body.String(), `"code":"action_rate_limited"`) || strings.Contains(issue.Body.String(), "secret") {
		t.Fatalf("rate response=%d %v %s", issue.Code, issue.Header(), issue.Body.String())
	}
