...
func TestA08ContractHasExactScopesAndStableResult(t *testing.T) {
	...
	assertContractKeys(t, contractAt(t, document, "common").(map[string]any), "authentication", "path_guid", "body_limit_bytes", "strict_json", "reason", "versions", "overrides", "response_headers", "mutation_replay", "eligibility")
	...
	assertContractKeys(t, contractAt(t, document, "common", "versions").(map[string]any), "expected_auth_version", "expected_permissions_version", "catalog_version")
	...
	assertContractValue(t, document, "complete array of unique capabilities, each item an exact object with only capability and effect; effect is inherit, allow, or deny; canonical persistence omits inherit; reject unknown, unavailable, duplicate, case-folded duplicate, and allow on Root-only or ungrantable capabilities", "common", "overrides")
	...
	assertContractKeys(t, contractAt(t, document, "common", "eligibility").(map[string]any), "actor", "promote_target", "demote_target", "permissions_target", "invisible_target", "visible_but_forbidden", "stale_or_same_state")
	...
	for _, tc := range []struct {
		...
		t.Run(tc.scope, func(t *testing.T) {
			...
			assertContractKeys(t, contractAt(t, action, "issue").(map[string]any), "method", "path", "forbidden_headers", "body", "response_status", "response_keys", "ticket_ttl_seconds")
			assertContractValue(t, action, "POST", "issue", "method")
			assertContractValue(t, action, "/admin/v2/action-verifications", "issue", "path")
			assertContractKeys(t, contractAt(t, action, "issue", "body").(map[string]any), tc.issueBodyKeys...)
			...
			assertContractKeys(t, contractAt(t, action, "execute").(map[string]any), "method", "path", "required_headers", "header_rules", "body", "response_status", "response_keys", "success_rule", "processing_success_response")
			...
			assertContractKeys(t, contractAt(t, action, "execute", "header_rules").(map[string]any), "Idempotency-Key", "X-Action-Ticket")
			...
			assertContractKeys(t, contractAt(t, action, "execute", "body").(map[string]any), tc.executeBodyKeys...)
			...
			assertContractKeys(t, contractAt(t, action, "query").(map[string]any), "method", "path", "exact_scope", "query_rule", "required_headers", "forbidden_headers", "original_key_rule", "response_status")
	...
	assertContractKeys(t, contractAt(t, document, "operation_query").(map[string]any), "statuses", "response_keys", "nullable_result_rule", "state_rules", "retry_after", "visibility", "refresh")
	...
	stateRules := contractAt(t, document, "operation_query", "state_rules").(map[string]any)
	...
	for state, want := range map[string]map[string]any{
		...
		assertContractKeys(t, stateRules[state].(map[string]any), "finished_at", "failure_code", "result_keys", "Retry-After")
		if !reflect.DeepEqual(stateRules[state], want) {
			t.Fatalf("operation query state %s=%#v, want %#v", state, stateRules[state], want)
		}
	}
	assertContractValue(t, document, "processing only; integer seconds 1..30", "operation_query", "retry_after")

	assertContractKeys(t, contractAt(t, document, "errors").(map[string]any), "authentication_401", "envelope", "status_code_allowlist", "operation_failure_codes", "header_rules", "prohibited_fields")
	...
	assertContractKeys(t, contractAt(t, document, "frontend_security").(map[string]any), "memory_only", "prohibited_values", "secret_prohibitions", "mutation_auto_replay_count", "ownership")
...
func validateA08ContractGuard(contents []byte) error {
	...
	query, err := a08At(document, "actions", "users.promote", "query")
	if err != nil {
		return err
	}
	return a08ExactKeys(query.(map[string]any), "method", "path", "exact_scope", "query_rule", "required_headers", "forbidden_headers", "original_key_rule", "response_status")
}
