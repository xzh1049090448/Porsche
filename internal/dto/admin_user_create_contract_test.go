...
func TestAdminUserCreateContractExamples(t *testing.T) {
	...
	adminUserCreateRawValue(t, integrity, "A14 locks every snapshot by target_guid independently of mutable operation result fields, validates each HMAC and matching successful outbox marker, and atomically redacts all snapshots with the user; a resolved 0010 zero-HMAC migration sentinel is upgraded under an exact guarded update to the canonical current-key redacted HMAC in that transaction; a marker/response count mismatch fails closed while zero markers and zero snapshots means no create snapshot exists", "deletion_lookup")
	adminUserCreateRawValue(t, integrity, "0010 first normalizes every legacy redacted snapshot to fixed 201 application/json, canonical {}, its fixed digest, and a zero-HMAC sentinel regardless of its previous body or target; it adds durable outbox result_kind, backfills target_guid only from an exact successful live create operation or an exact successful ResultUser outbox bound by operation_id, action, public_ref, TargetNone, positive real-user result_guid, and active marker state for an expired operation; every mismatch or unresolved active snapshot is redacted with no recoverable PII before the final outcome/lifecycle CHECK, target index, and foreign key", "migration")
