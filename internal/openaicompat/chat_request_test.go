...
func TestDecodeChatNormalizesToolRoundTrip(t *testing.T) {
	...
	got, err := DecodeChat(body, NoReasoning)
...
func TestDecodeChatRejectsBrokenCallSequences(t *testing.T) {
	...
	for _, body := range inputs {
		if _, err := DecodeChat([]byte(body), NoReasoning); err == nil || err.Code != "invalid_request" {
...
func TestDecodeChatClassifiesOversizedHistoricalToolPayloads(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, got := DecodeChat(body, NoReasoning); got == nil || got.Status != 413 || got.Code != "request_too_large" {
...
func TestDecodeChatKeepsMalformedToolPayloadsAt400(t *testing.T) {
	for _, body := range [][]byte{
		...
		if _, got := DecodeChat(body, NoReasoning); got == nil || got.Status != 400 || got.Code != "invalid_request" {
