...
func TestPayOrderConcurrentSettlement(t *testing.T) {
	...
	phone := testPhone()
	user := models.User{AuditFields: models.AuditFields{Guid: generator.Next(), CreatedAt: now, UpdatedAt: now, IsDeleted: 0}, GroupID: testDefaultBusinessGroupID(t, db), Phone: &phone, Status: models.UserStatusActive, PlanType: models.PlanFree, AllowedModels: models.JSONSlice{}}
