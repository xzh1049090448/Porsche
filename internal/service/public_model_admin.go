...
func projectPublicModel(m models.PublicModelConfig) PublicModelAdmin {
	return PublicModelAdmin{GUID: fmt.Sprint(m.Guid), ModelKey: m.ModelKey, UpstreamModelID: m.UpstreamModelID, DisplayName: m.DisplayName, Provider: m.Provider, Capabilities: clonePublicStrings(m.Capabilities), ContextWindow: m.ContextWindow, InputPriceUSDPerMillionTokens: m.InputPriceUSDPerMillionTokens, OutputPriceUSDPerMillionTokens: m.OutputPriceUSDPerMillionTokens, Status: m.Status.String(), Revision: m.Revision, LastUpstreamCheckAt: m.LastUpstreamCheckAt, PublicDisplayGroup: m.PublicDisplayGroup, EndpointTypes: clonePublicStrings(m.EndpointTypes), PublicRestrictions: clonePublicStrings(m.PublicRestrictions), PriceSource: m.PriceSource, PriceReviewer: m.PriceReviewer, PriceEffectiveAt: m.PriceEffectiveAt}
}
