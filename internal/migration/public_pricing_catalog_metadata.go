...
func VerifyPublicPricingCatalogMetadataSchema(ctx context.Context, db *gorm.DB) error {
	...
	if err := verifyPublicPricingCatalogMetadataTable(ctx, db, "public_model_configs"); err != nil {
		return err
	}
	return verifyPublicPricingCatalogMetadataTable(ctx, db, "public_price_snapshot_items")
}

func verifyPublicPricingCatalogMetadataTable(ctx context.Context, db *gorm.DB, table string) error {
	var count int64
	var query string
	var expected int64
	switch table {
	case "public_model_configs":
		expected = 6
		query = `SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='public_model_configs' AND (
...
(column_name='price_effective_at' AND column_type='bigint' AND is_nullable='YES' AND column_default IS NULL AND character_set_name IS NULL AND collation_name IS NULL))`
	case "public_price_snapshot_items":
		expected = 7
		query = `SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='public_price_snapshot_items' AND (
...
(column_name='effective_at' AND column_type='bigint' AND is_nullable='YES' AND column_default IS NULL AND character_set_name IS NULL AND collation_name IS NULL))`
	...
	if err := db.WithContext(ctx).Raw(query).Scan(&count).Error; err != nil || count != expected {
		return ErrPublicPricingCatalogMetadataMigration
	}
	return nil
}
