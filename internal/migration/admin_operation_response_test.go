...
func TestAdminOperationResponseMigrationIsOrderedAndChecksumProtected(t *testing.T) {
	migrations, err := All()
	if err != nil {
		t.Fatal(err)
	}
	if len(migrations) < 10 || migrations[8].Version != "0009" || migrations[9].Version != "0010" {
		t.Fatalf("All() = %#v, want 0009 and 0010 at their published positions", migrations)
	}
