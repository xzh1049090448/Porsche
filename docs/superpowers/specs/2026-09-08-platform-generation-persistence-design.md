...
MySQL commit can return an uncertain transport error. The service must not blindly replay the callback. It resolves the outcome by querying the receipt with `(user_id, generation_id)` on a fresh database operation: an active, internally consistent receipt means committed success; absence means no proven success and returns a stable unavailable result for recovery. A malformed or partial receipt fails closed as an integrity error.
