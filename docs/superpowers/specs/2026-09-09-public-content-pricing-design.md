...
The monitor accepts a catalog only after a 2 MiB plus one-byte bounded read proves the response is within the limit and strict decoding proves exactly one complete JSON document with the trusted field schema. The root must contain exactly one canonical `data` array and one canonical boolean `complete`; every model item must contain one canonical normalized valid string `id`. Unknown, missing, case-variant, or duplicate fields, trailing documents or garbage, malformed field types, invalid UTF-8, and truncation fail closed. An authenticated explicit `data: []` with `complete: true` is a valid complete empty catalog and therefore distinct from an omitted `data` field. A later successful complete fresh catalog resolves the active catalog-sync failure in the same transaction as its observations and lifecycle updates.
