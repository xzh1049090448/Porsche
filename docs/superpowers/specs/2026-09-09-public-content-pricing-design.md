...
The monitor accepts a catalog only after a 2 MiB plus one-byte bounded read proves the response is within the limit and strict decoding proves exactly one complete JSON document with the trusted field schema. Unknown or duplicate fields, trailing documents or garbage, malformed field types, and truncation fail closed. A later successful complete fresh catalog resolves the active catalog-sync failure in the same transaction as its observations and lifecycle updates.
