...
The approved source requirement is Porsche-Web's `2026-09-07-chat-adaptive-character-streaming-prd.md` v1.0. The currently inspected Porsche-Web `interface-contract.json` is version `v1.0.0`, status `draft`, SHA-256 `0891e452f122922f576745db89c96c853a9a7cf4ff00078c30ae3b3f0769e970`, and does not contain the streaming endpoints. It is evidence of an unresolved cross-repository handoff, not joint contract acceptance. BE06 may implement the already approved PRD, but joint acceptance must later bind a versioned interface contract and exact frontend/backend revisions.
...
V2-only controls are stripped before constructing each upstream request. Validation and authorization finish before claim, quota mutation, response headers, persistence, or upstream access. Legacy compare streaming, non-streaming compare, single v2, GET/cancel, authentication, ACL, and public error envelopes remain behaviorally compatible.
...
- one thread-safe `PlatformSSEV2Encoder` and one detach-on-error output writer;
- one serialized owner-bound Redis mutation boundary shared by model workers and renewal;
...
Workers may receive and process chunks concurrently. Cross-model events may interleave, but the shared encoder and output writer serialize complete SSE frames. Within one model, a non-empty UTF-8 delta follows this order:

1. validate the typed upstream chunk, model identity, content limit, token fields, and next sequence;
2. perform owner-bound `RecordDeltaOwned` under the shared mutation boundary;
3. update the worker's in-memory state;
4. encode and attempt to emit the `delta` frame.

A worker that reaches a valid upstream terminal performs owner-bound `MarkModelDoneOwned` and emits one `model_done`. A model-local upstream timeout, error, malformed stream, invalid chunk, or content overflow performs a new owner-bound `MarkModelFailedOwned` transition with a stable code and emits one `model_error`. A model-local failure never cancels a different model.
...
After the receipt is validated, Redis `Complete` receives assistant GUIDs for successful models only. If the completion result is unknown, the coordinator runs receipt-backed `ReconcileComplete` and never replays a potentially committed SQL transaction. It then loads the user's authoritative total token count and emits the unique global `done`. The `models` object reports each requested model as `completed` with tokens or `failed` with its stable code.
...
- `committing` is reconciled from the receipt graph;
...
Once streaming starts, the response uses the existing v2 headers and exactly one `meta`. The only body event types are `meta`, `delta`, `model_done`, `model_error`, `done`, and global `error`; v2 never emits bare `[DONE]`. A post-header failure is expressed only through sanitized SSE when the writer remains attached. EOF or writer failure is not treated as business success.
...
Implementation follows TDD. Focused unit and race tests cover validation before side effects, exact duplicate behavior, two/three-model fan-out, independent fast/slow workers, cross-model interleaving, per-model strict sequence, partial failure, all-failed behavior, detached output, cancel/commit races, renewal authority, commit-unknown reconciliation, shared mutation serialization, response-body closure, admission drain, and shutdown.
...
8. concurrent duplicate and quota races.
