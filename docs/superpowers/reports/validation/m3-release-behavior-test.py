...
SRC=Path(__file__).with_name('m3-release-executed.py').read_text(); tree=ast.parse(SRC)
...
for case in cases:
 with tempfile.TemporaryDirectory(prefix='m3-release-test-') as folder:
  ...
  old={'Id':ns['OLD'],'Image':ns['OLDIMAGE'],'Name':'/ai-gateway-go','State':{'Running':True},'Mounts':[], 'Config':{'Hostname':'fixture','Domainname':'','User':'','Env':['PRIVATE_SENTINEL=DO_NOT_PRINT','APP_ENV=production'],'Cmd':['./server'],'WorkingDir':'/app','Volumes':None,'Labels':{}},'HostConfig':{'NetworkMode':'porsche-app','PortBindings':{'8000/tcp':[{'HostIp':'127.0.0.1','HostPort':'8000'}]},'RestartPolicy':{'Name':'no','MaximumRetryCount':0},'AutoRemove':False},'NetworkSettings':{'Networks':{'porsche-app':{}}}}
