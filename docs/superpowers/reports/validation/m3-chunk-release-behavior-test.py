...
SRC=Path(__file__).with_name('m3-chunk-release-prepared.py').read_text(); tree=ast.parse(SRC)
main=next(i for i,n in enumerate(tree.body) if isinstance(n,ast.Try))
manifest=json.loads(Path(__file__).with_name('m3-chunk-diagnostic-candidate.json').read_text())
