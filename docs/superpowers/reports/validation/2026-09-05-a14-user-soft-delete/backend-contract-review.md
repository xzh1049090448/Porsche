...
401 继续由现有认证中间件产生，使用只有 `detail` 的 legacy envelope，不公开 `code`。其余 A14 handler 错误使用固定 `admin_action_error`；只有 `operation_commit_unknown` 可公开 `operation_ref`。
...
`internal/handler/admin_user_actions_contract_runtime_test.go` 通过生产路由注册 helper、真实 handler 和真实 `RequireUser` 中间件，将 Issue、Execute、Query 成功响应及未认证 401 直接绑定到冻结合同。legacy 410 测试从同一合同读取路由、header、状态和响应 fixture，使用与生产完全相同的认证后 handler 链验证零业务依赖。
