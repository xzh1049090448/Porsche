...
- Use TDD for every behavior change. Do not alter any committed migration 0001–0009 or its checksum; response-target remediation must use new migration 0010.
...
For create execution, validate the initial password as bytes, then create two fully independent owned backing arrays before `Operation.Begin`: one belongs only to the canonical intent and may be cleared by `Begin`/descriptor encoding, while the other is retained only for a possible fresh hash. `Operation.Begin` completes authentication, action rate limiting, role/capability checks, and terminal idempotency replay first. Forbidden, rate-limited, error, and replay outcomes hash zero times and clear both applicable buffers. Only when `ExecutionReady` returns true may the handler hash the independent buffer exactly once, immediately clear it, clear the intent buffer, and pass only the owned hash plus safe request metadata to `CreateAccountExecution`. For admin verification, the initial password and current actor password remain separate owned buffers and both are cleared after `Issue`; byte verification does not construct a plaintext password string.
...
- [ ] **Step 1: Provision task-owned fixtures and run migration 0001–0010**
...
```bash
go test ./internal/migration ./internal/actionsecurity ./internal/service ./internal/dto ./internal/handler ./internal/router -run 'BusinessGroup|CreateAccount|AdminUserCreate|ActionOperation|AdminOperationResponse|DeleteUser' -count=1
go test -race ./internal/actionsecurity ./internal/service ./internal/handler -run 'CreateAccount|AdminUserCreate|ActionOperation|DeleteUser' -count=1
go test ./... -count=1
go build ./...
go vet ./...
git diff --check
