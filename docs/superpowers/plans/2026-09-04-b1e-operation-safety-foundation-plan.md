...
type OperationIdentity struct { ID int64; PublicRef string; actor ActionActor; capability *operationLeaseCapability }
...
Treat the identity as a one-shot capability shared by every shallow copy. Its only exported fields are ID and PublicRef; exact JSON is only `public_ref`, while the lease owner, private actor claims, capability mutex, and consumed state are excluded. Begin stores the raw lease in an unexported shared `operationLeaseCapability`. Execute atomically takes and zeros that shared capability before Redis, MySQL, or callback work; only one concurrent caller can obtain a local lease, all other original/copy/manual/zero identities fail closed without entering the consumer. Never restore the capability on validation failure, Redis rejection/failure, success, typed failure, infrastructure failure, or commit unknown, and clear the local lease before returning. Open a new owned GORM transaction. Lock actor, session, operation, verification in order; compare lease owner HMAC constant-time; revalidate state/auth/intent/session; conditionally consume verification with `consumed_at IS NULL AND expires_at > now AND is_deleted=0`, setting `consumed_at=now`, `is_deleted=1`, and update audit fields; invoke consumer, audit, and outbox in that same transaction. Known typed rejection rolls back callback savepoint, then records failed operation + rejection audit/outbox in the same outer transaction. Infrastructure error rolls back everything. Success writes finished/query expiry/result and clears the persisted lease.

- [ ] **Step 4: Make commit unknown explicit**

Use an injectable transaction runner in tests so a commit can return an error after the callback. The shared lease capability was already atomically consumed before the transaction and remains consumed when returning `CommitUnknownError{PublicRef: ...}` mapped later to 503; do not restore it, open a second transaction, update state outside the transaction, or retry callback. A subsequent Query is the only confirmation route.

- [ ] **Step 5: Add the complete fault matrix**

Inject failure at actor lock, session lock, operation lock, verification lock, ticket consume, fixture effect, audit, outbox, terminal update, and commit return. Assert no partial effect, no false terminal success/failure, no callback replay, and no secrets in errors. Add succeeded and typed failed atomic cases. For success, known failure, prevalidation rejection, Redis revoked/error, database lock error, terminal update error, and commit unknown, assert the shared capability is consumed with raw bytes zero. Assert shallow copies cannot replay, two concurrent Execute calls enter the consumer at most once under `-race`, and value/pointer formatting plus nested/value/pointer JSON disclose only `public_ref`.
