...
- [ ] 实现者按init.sh核对基础；先以合成坏帧断言新增分类、观察缺失分类的RED。
- [ ] 最小实现固定分类/位置与日志入口白名单，保持malformed_chunk及旧公开输出；正常和无trace无额外日志。
- [ ] 对所有现有拒绝条件及正常边界运行go test ./internal/diagnostics ./internal/whitelabel -count=1，保存实际命令输出。
- [ ] 协调者使用显式隔离TEST_DATABASE_URL/TEST_REDIS_URL执行go test -json -p 1 ./... -count=1；专项-race、go vet ./...、go build ./...、git diff --check。
- [ ] 后端PM复核规格，独立质量以新构造输入验证脱敏和接受/拒绝行为对照。
- [ ] 按精确ID和所有权标签清理本轮测试容器及临时测试凭据，更新进度与双方交接。
- [ ] 冻结代码提交，精确独立clone交叉构建及离线镜像验证，提交具体候选/回滚材料；未授权的新候选不上传部署。
